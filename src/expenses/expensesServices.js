...
const { getUserById } = require('../users/usersServices');
...
const validateData = (request) => {
  ...
  const { userId, spentAt, title, amount, category } = request;
...
const getAllExpenses = ({ userId, categories, from, to }) => {
  expenses = expenses.filter((expense) => {
    const userIdMatch = userId ? +expense.userId === +userId : true;
...
const create = ({
  ...
  const maxId
  = expenses.length > 0 ? Math.max(...expenses.map(({ id }) => id)) : 0;
